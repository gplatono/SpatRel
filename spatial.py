...
class Spatial:
	...
	def update(self, world):
		...
		# self.vox = Voxel(scope=self.world.entities, depth=2)
	...
	def where(self, tr):
		...
		entities = self.world.active_context
		entity_pairs = [(ent1, ent2) for (ent1, ent2)\
							in list(itertools.combinations(entities, r = 2))\
								if tr.name != ent1.name and tr.name != ent2.name]

		def get_vals(pred_func):
			if pred_func != self.between:
				val = [((tr, ent), pred_func(tr, ent)) for ent in entities]
			else:
				val = [((tr, ent1, ent2), between(tr, ent1, ent2)) for (ent1, ent2) in entity_pairs]
		...
		pred_list = [item.compute for item in pred_list]
...
class ArgumentRank(Node):
	"""
	Computer the rank of the trajector w.r.t. the relation and landmarks.
	"""

	def __init__(self, network=None):
		self.network = network

	def compute(self, tr, relation, lms):
		ranks = self.network.rank_trs(relation, lms)		
		for idx in range(len(ranks)):
			if ranks[idx][0] == tr:
				return idx+1

	def str(self):
		return 'argument_rank.n'
...
class VerticalDeicticComponent(Node):
	def compute(self, tr, lm):
		axial_dist = self.network.axial_distances[(tr, lm)]
		return math.exp(- torch.tensor(1.0, dtype=torch.float32, requires_grad=True) * math.fabs(axial_dist[1]))
...
class Touching(Node):
	...
	def compute(self, tr, lm):
		...
		#final_score = torch.tensor(float(self.network.vox.contains([tr, lm], depth=2)), dtype=torch.float32, requires_grad=True)
		overlap = tr.bvh_tree.overlap(lm.bvh_tree)
		print (tr.name, lm.name, overlap)
		final_score = 1 if len(overlap) > 0 else 0
		final_score = torch.tensor(final_score, dtype=torch.float32, requires_grad=True)
		#final_score = torch.tensor(float(len(), dtype=torch.float32, requires_grad=True)
		#print ("F SCORE: ", tr.name, lm.name, final_score)
		# for part_tr in tr.components:
		# 	for part_lm in lm.components:
		# 		part_score = torch.tensor(float(self.network.vox.contains([part_tr, part_lm], depth=2)), dtype=torch.float32, requires_grad=True)
...
class RightOf_Deictic(Node):
	...
	def __init__(self, connections):
		self.connections = connections
		self.connections['rightof_deictic_pure'] = RightOf_Deictic_Pure(connections)
		self.connections['argument_rank_factor'] = (connections)
	...
	def compute(self, tr, lm):
		...
		# self.factors["hor_component"] = horizontal_component = self.connections['horizontal_deictic_component'].compute(tr, lm)
		# self.factors["ver_component"] = vertical_component = self.connections['vertical_deictic_component'].compute(tr, lm)

		# weighted_sum = self.parameters["hor_weight"] * horizontal_component + self.parameters["ver_weight"] * vertical_component
		
		# final_score = 1 / (1 + math.e ** (- torch.abs(self.parameters['sigmoid_decay']) * weighted_sum))
		pure_score = self.connections['rightof_deictic_pure'].compute(tr, lm)

		final_score = pure_score
		#TODO!!!
		return final_score
...
class RightOf_Deictic_Pure(Node):
	"""Deictic sense of the to-the-right-of relation."""
	...
	def str(self):
		return 'to_the_right_of_deictic_pure.p'
...
class InFrontOf_Extrinsic(Node):
	def __init__(self, connections, network):
		...
		self.parameters = {"centroid_weight": torch.tensor(0.01, dtype=torch.float32, requires_grad=True),
						   ...
						   "cone_width": torch.tensor(0.7, dtype=torch.float32, requires_grad=True),
						   "dist_decay_param": torch.tensor(0.7, dtype=torch.float32, requires_grad=True)}
	...
	def compute(self, tr, lm):
		...
		extrinsic_front = self.network.world.front_axis
		#cos = extrinsic_right.dot(disp_vec)

		within_cone_factor = self.connections['within_cone_region'].compute(disp_vec, extrinsic_front, self.parameters['cone_width'])
		scaled_dist_factor = dist / (max(tr.size, lm.size) + 0.001)
		distance_decay_factor = math.e ** (- torch.abs(self.parameters['dist_decay_param']) * scaled_dist_factor)
		#print ("WITHIN CONE EXTR: ", within_cone_factor, "DIST: ", scaled_dist_factor)
		final_score = within_cone_factor * distance_decay_factor
...
class Clear(Node):
	def __init__(self, connections=None, network=None):	
		self.arity = 1
		self.network = network

	def compute(self, tr):
		max_val = 0
		for entity in self.network.world.active_context:
			max_val = torch.max(max_val, self.network.on(entity, tr))
