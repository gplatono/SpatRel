...
class Voxel:
	def __init__(self, scope, location=None, size=None, parent=None, depth=0, child_idx=[None, None, None]):
		...
		for entity in scope:
			...
			# 	print (self.location, entity.name, tree.find_nearest(self.location))
			# 	if len(tree.find_nearest(self.location)) > 0:#tree.overlap(self.bvh_tree) != []:
			# 		self.intersect_list.append(entity)
			# 		break
...
vox = Voxel(scope = world.entities, depth=3)
vox.fillNeighbors()
vox.print_self()
